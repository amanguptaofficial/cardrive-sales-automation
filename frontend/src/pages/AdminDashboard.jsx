...
import { Shield, UserCheck, X, Check, Trash2, Mail, Phone, LogOut, Menu, X as CloseIcon } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const adminAgent = JSON.parse(localStorage.getItem('adminAgent') || '{}');
  ...
  }, [adminToken, adminAgent, navigate]);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div className={`bg-gray-900 text-white min-h-screen flex flex-col transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} relative`}>
          ...
          <div className={`flex items-center ${sidebarOpen ? 'justify-start gap-2' : 'justify-center'}`}>
            ...
            {sidebarOpen && (
              <>
                ...
                <span className="text-xs bg-accent px-2 py-0.5 rounded">Portal</span>
              </>
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 p-4">
          <div>
            {sidebarOpen && (
              <div className="text-xs text-gray-400 uppercase tracking-wider mb-2 px-2">MANAGEMENT</div>
            )}
            <button
              onClick={() => setFilter('pending')}
              className={`w-full flex items-center ${sidebarOpen ? 'gap-3' : 'justify-center'} px-3 py-2 rounded-lg mb-1 transition-colors ${
                filter === 'pending'
        ...
        <div className="p-4 border-t border-gray-800">
          <div className={`flex items-center ${sidebarOpen ? 'gap-3' : 'justify-center'} mb-3`}>
            <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center text-white font-bold flex-shrink-0">
              {adminAgent?.name?.charAt(0) || 'A'}
            </div>
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{adminAgent?.name || 'Admin'}</div>
                <div className="text-xs text-gray-400 capitalize truncate">
                  {adminAgent?.role?.replace('_', ' ') || 'Admin'}
                </div>
              </div>
            )}
          </div>
          <button
            ...
            className={`w-full flex items-center ${sidebarOpen ? 'gap-2' : 'justify-center'} px-3 py-2 text-gray-300 hover:bg-gray-800 rounded-lg transition-colors`}
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="bg-white border-b border-gray-200 px-6 py-4">
          ...
          <p className="text-gray-600 mt-1">Manage user signups and permissions</p>
        </div>
